...
public class WebSocketEventListener {
    ...
    public List<ChatMessage> lastMessage(final Map<String, String> payload) {
        String messageId = payload.get("messageId");
        if (messageId != null) {

            return chatMessageRepository.findTop10ByMessageId(messageId);
        } else {
            if (log.isInfoEnabled()) {
                log.error("Received null messageId in lastMessage payload");
            }
            return null;
        }
    }
