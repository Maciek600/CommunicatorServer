...
//    /**
//     * Handles adding a user to the chat. Associates the username with the WebSocket session.
//     *
//     * @param chatMessage    the message payload containing the username.
//     * @param headerAccessor the accessor for WebSocket headers.
...
//    }
    ...
    public ResponseEntity<List<ChatMessage>> getOlderMessages(
        ...
        List<ChatMessage> messages = chatMessageRepository.findMessagesBefore(chatId, messageId, Pageable.ofSize(size));
        return ResponseEntity.ok(messages);
    }
//    public ResponseEntity<ChatMessagePageDto> getChatMessage(final @RequestParam String query,
//                                                             final @RequestParam(defaultValue = "1") int page,
//                                                             final @RequestParam(defaultValue = "10") int size) {
//        Page<ChatMessage> foundMessages = chatMessageService.getChatMessages(query, page, size);
//        if (foundMessages.getTotalElements() > 0) {
//            ChatMessagePageDto chatMessagePageDto = new ChatMessagePageDto();
//            chatMessagePageDto.setResults(foundMessages);
//            return ResponseEntity.ok(chatMessagePageDto);
//        }
//    }
